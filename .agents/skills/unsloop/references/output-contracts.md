...
When persistent files were written, report the selected write policy only as needed for review. Under Immutable versions, identify the response batch and history location, included files, and any incomplete snapshot or collision. Under Overwrite current, do not imply that response-level history exists. Keep this after the artifact and do not let version bookkeeping dominate the handoff.
...
For website and digital-content work, identify whether the site is target artifact, evidence source, or both; state the live or captured version, acquisition method, page/family/journey/section/sample/enumerated-site boundary, inspected and inaccessible states, material strengths, page and cross-page effects, and validation actually performed. Label Content findings, Technical findings, Behavioral findings, and Hypotheses separately. A sampled Audit remains sample-bound and confirms that the live site was not changed.

...
- [references/web-content.md](references/web-content.md) when a website, web page, page family, user journey, web application surface, help center, or content portal is the artifact being written, reviewed, or audited rather than merely a research source;
...
8. Apply the human-voice lens to existing prose and before finalizing new prose. Test whether examples perform a clear function and whether emotional force is earned rather than manufactured. For multi-section work, also test the closing passage, heading or break, and next opening as one logical boundary. For spoken work, trace the active question, contrast, consequence, or repeated key term that carries listeners from one thought to the next, and check that named subjects and pronouns remain clear. Read [references/human-voice-review.md](references/human-voice-review.md) and, when applicable, [references/section-flow.md](references/section-flow.md).
...
- In spoken or presented work, preserve genuine thought chains: a question, contrast, consequence, or key term may carry the next sentence, reading, or section. Prefer that reasoning to structural labels alone, but use orientation labels when they materially help listeners.

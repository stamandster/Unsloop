...
| FS-006 | Human-voice and section-flow analysis | Draft, writing brief, optional voice brief, visible section boundaries, spoken thought sequence | Voice/slop, example, emotional-integrity, cross-section-flow, oral-continuity, and reference-clarity findings | PR-007, PR-008 |
...
| FS-044 | Delivery and presentation orchestration | Delivered-writing request, brief, governing directions, timing or attention constraints, audience, spoken thought sequence, evidence, questions, media, interaction, closing goal | `DeliveryContract`, reconciled element plan, audible reasoning and reference-clarity findings, delivery-aware artifact or findings, and bounded readiness | PR-045; NFR-003, NFR-020 |
...
6. Make paragraph-scale spoken movement audible through the smallest earned question, contrast, consequence, or recurring key term. Treat headings and ordinal labels as orientation when useful, not as substitutes for reasoning. Check named-subject and pronoun clarity without overriding a deliberate personal choice that serves the speaker-audience relationship.
7. Review material questions for function, placement, answer path, pause or response, and payoff; never impose a question quota.
8. For mixed audiences, provide a shared entry point, explain material unfamiliar concepts, and offer useful depth without flattening or singling out groups unnecessarily.
9. Give consequential optional media or interaction a compact decision brief covering placement, audience perception, function, intended response, handling, cost, accessibility, recommendation, and alternative. Keep it unselected and readiness Provisional while a material choice remains open.
10. Make the closing establish the intended final audience state, resolve the governing movement, and offer action only when appropriate without introducing a new major point.
11. Recalculate the complete audience-time cost after oral-flow changes add or remove questions, pauses, examples, or connective language.
12. In Review, diagnose only what the inspected artifact and delivery evidence support. In Audit, preserve every inspected artifact unchanged. In Write, implement only confirmed delivery and media decisions.
...
| Spoken section uses headings and ordinal labels without a thought chain | PR-007, PR-045 / FS-006, FS-044 | Review traces the missing relationship, tests question/contrast/consequence/key-term continuity and named-subject/pronoun clarity, preserves purposeful pauses, proposes the smallest earned connection, and recalculates timing after revision. |
